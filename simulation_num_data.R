...
Var.Y <- sd.ny^2 + sd.nx^2 * t(alpha) %*% alpha + sd.nz^2 * t(gamma + t(B) %*% alpha) %*% (gamma + t(B) %*% alpha)
sd.O <- sqrt(Var.Y - t(Cov.Y.X) %*% solve(Cov.X) %*% Cov.Y.X)
...
n <- c(300, 900, 1800, 2700, 3600, 4500, 5400)
# num interventional data
m <- c(100, 300, 600, 900, 1200, 1500, 1800)

# store MSEs and SDs
mses <- matrix(numeric(length(n)*8), ncol=8)
sds <- matrix(numeric(length(n)*8), ncol=8)

for (size in 1:length(n)) {
  print(paste("loop: ", size))
  for (it in 1:num.it) {
    # generate data from structural causal model
    data.O <- generate_O(n=n[size], p=p, sd.nz=sd.nz, sqrt.cov.x = cov.mat.x.o.sqrt, alpha=alpha, gamma=gamma, B=B)
    X.O <- data.O[[1]]
    Y.O <- data.O[[2]]
    
    data.I <- generate_I(n=m[size], p=p, sd.nz=sd.nz, sqrt.cov.x = cov.mat.x.i.sqrt, alpha=alpha, gamma=gamma, B=B)
    X.I <- data.I[[1]]
    Y.I <- data.I[[2]]
  ...
  mses[size, 1] <- mean(errs.l2)
  sds[size, 1] <- sd(errs.l2)
  mses[size, 2] <- mean(errs.l1)
  sds[size, 2] <- sd(errs.l1)
  mses[size, 3] <- mean(errs.standard)
  sds[size, 3] <- sd(errs.standard)
  mses[size, 4] <- mean(errs.I)
  sds[size, 4] <- sd(errs.I)
  mses[size, 5] <- mean(errs.pool)
  sds[size, 5] <- sd(errs.pool)
  mses[size, 7] <- mean(errs.rosenman)
  sds[size, 7] <- sd(errs.rosenman)
  mses[size, 8] <- mean(errs.optimal)
  sds[size, 8] <- sd(errs.optimal)
}